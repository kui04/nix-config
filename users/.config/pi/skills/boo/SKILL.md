...
- `boo new <name> -d -- <command>` — start a detached session running `<command>`. Omit `-d` and the command to get an attached session running `$SHELL` (rarely what you want from an agent). Never omit `<name>` — an auto-derived name won't carry the `pi-` prefix.
