{
  ...
  serviceFiles = lib.mapAttrs' (name: text:
    lib.nameValuePair "${managedUnitsDir}/${name}.service" {inherit text;})
  cfg.services;
in {
  ...
  config = {
    ...
    home.activation.switchSystemdSystemServices = lib.hm.dag.entryAfter ["linkGeneration" "decryptAgenix"] ''
