...
in {
  ...
  home.activation.installXrayService = lib.hm.dag.entryAfter ["decryptAgenix"] ''
    $DRY_RUN_CMD install -Dm644 ${config.home.file.".config/systemd-services/xray.service".source} /etc/systemd/system/xray.service
    $DRY_RUN_CMD /bin/systemctl daemon-reload
    $DRY_RUN_CMD /bin/systemctl enable --now xray.service
    $DRY_RUN_CMD /bin/systemctl restart xray.service
  '';
}
